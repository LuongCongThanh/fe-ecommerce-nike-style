...
- **Access token lưu `localStorage`/`sessionStorage`**: bị từ chối, vi phạm trực tiếp nguyên tắc bảo mật đã chốt trong `authentication-authorization.md` (không lưu token dài hạn ở `localStorage`).
