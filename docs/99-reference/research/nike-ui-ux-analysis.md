...
- Dùng file này như tài liệu tham chiếu về pattern UI/UX, không phải blueprint để sao chép nguyên xi.
- Không dùng file này để chốt scope, kiến trúc hay component list cuối cùng cho dự án.
- Ưu tiên đọc các phần đầu nếu cần hiểu cấu trúc trải nghiệm tổng thể.
- Chỉ tra các phần sâu hơn khi đang thiết kế một khu vực cụ thể như header, hero, PDP, search hoặc navigation.
- Khi triển khai thật, luôn đối chiếu lại với định hướng sản phẩm và kiến trúc trong `docs/`.

## Đọc nhanh trước

- `1. Kiến trúc tổng thể của Website`: nắm cấu trúc trải nghiệm toàn trang.
- `3. Header của Nike`: hiểu cách Nike giảm cognitive load và dẫn hướng người dùng.
- `4. Hero Section`: xem cách thương hiệu ưu tiên storytelling trước bán hàng.
- Các phần sau phù hợp để tra cứu theo chủ đề, không cần đọc liền mạch từ đầu đến cuối.

## Nên mượn gì, không nên bê nguyên gì

- Nên mượn: cách kể chuyện ở hero, cách giữ menu gọn, cách ưu tiên hình ảnh sản phẩm, nhịp typography lớn, và tư duy mobile-first.
- Không nên bê nguyên: sitemap, component list, cấu trúc thư mục, hoặc mọi quyết định kỹ thuật như thể đó là blueprint của dự án này.
- Khi một pattern nhìn hấp dẫn nhưng làm scope phình lên, quay lại `requirements/`, `product/`, và `architecture/` trước khi nhận nó vào backlog.
...
Nike dùng 12-column grid để giữ bố cục ổn định trên những trang có nhiều khối nội dung và hình ảnh lớn.
...
Header khá thấp và gọn, khoảng 90px, nên ưu tiên tốc độ định hướng hơn là nhồi thêm tính năng.
...
Nike giữ menu chính ở 5 mục. Cách này giúp giảm tải nhận thức và làm rõ rất nhanh website đang bán gì:
...
Người dùng gần như hiểu ngay cấu trúc danh mục mà không phải dò tìm quá nhiều.

## 4. Hero Section

Hero là vùng kể chuyện thương hiệu trước khi bán hàng. Cấu trúc thường chỉ cần bốn phần: media, heading, CTA và liên kết sang collection.
...
Điểm đáng mượn ở đây không phải số lượng nút, mà là cách tách một CTA cho giao dịch và một CTA cho khám phá. ([Nike.com][1])
...
Card chỉ giữ những thông tin đủ để quyết định có đáng bấm vào PDP hay không: ảnh, badge, tên, loại sản phẩm, số màu, giá.

Hiệu ứng hover đổi ảnh chỉ là phần tăng cảm giác khám phá; lõi UX vẫn là quét nhanh và so sánh nhanh.

## 6. Product Listing Page

PLP của Nike khá rõ vai: filter, sort, product grid và phân trang.
...
Mobile: filter và sort được nén thành hai nút riêng, để lưới sản phẩm giữ diện tích hiển thị. ([Nike.com][2])

## 7. Search UX

Nike ưu tiên predictive search hơn là bắt người dùng đi qua một search page nặng nề.
...
Cái đáng học ở đây là rút số bước tìm kiếm, không phải cố sao chép nguyên giao diện search của Nike.

## 8. Typography — Kích thước sử dụng trên site

Nike dùng ít kiểu chữ và dựa nhiều vào chênh lệch kích thước hơn là chênh lệch font-family.
...
Phần lớn giao diện chỉ dùng đen, trắng và xám, để sản phẩm và ảnh giữ vai trò nổi bật nhất.

## 10. Design System — Kiểm kê Foundation & Component

Danh sách dưới đây hữu ích như checklist tham khảo, không phải danh sách bắt buộc phải có ngay từ đầu.
...
Phần này chỉ nên xem như ví dụ tư duy phân nhóm. Dự án hiện tại đã có quy ước kiến trúc riêng trong `docs/architecture/frontend/`.
...
Điểm mạnh của Nike nghiêng về branding và UX nhiều hơn e-commerce thuần túy.
...
Nếu đọc tới đây, điều đáng giữ lại không phải là ý tưởng "clone Nike", mà là bộ nguyên tắc có thể tái dùng: ít màu, nhịp typography rõ, hero kể chuyện, search ngắn bước, và product card quét nhanh.
