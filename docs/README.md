...
1. **[`glossary.md`](./glossary.md)** — Domain glossary (ubiquitous language). Định nghĩa thuật ngữ domain, dùng thống nhất khắp dự án.
2. **[`adr/`](./adr/)** — Architecture Decision Records cho các quyết định gây tranh cãi/có trade-off rõ, kèm alternatives đã cân nhắc.
3. **[`planning/decision-log.md`](./planning/decision-log.md)** — Nhật ký mọi quyết định đã chốt (#1–33), không riêng những cái đủ lớn để thành ADR.
4. **[`architecture/frontend/`](./architecture/frontend/README.md)** — Hình thức hoá các quyết định trên thành tài liệu kiến trúc triển khai được. Không được mâu thuẫn với 3 nguồn trên.
5. **[`planning/brainstorm-session.md`](./planning/brainstorm-session.md)** — Bối cảnh/quá trình brainstorm (Understanding Summary, Assumptions, Open Questions) dẫn tới các quyết định. Nguồn ngữ cảnh, không phải nguồn quyết định.
6. **[`planning/reference/`](./planning/reference/README.md)** và **[`research/`](./research/)** — Tài liệu tham khảo thêm, **không phải quyết định**. Chỉ dùng làm gợi ý khi không mâu thuẫn với các nguồn trên.
...
├── architecture/
│   └── frontend/                         # thiết kế solution chính thức, triển khai được
├── planning/
