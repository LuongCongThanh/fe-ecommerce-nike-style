...
export interface CreateStaffAuthModuleOptions {
  ...
  /** The app's own typed navigate function (e.g. wrapping `useNavigate()` from
   * `@tanstack/react-router`, or `@/i18n/navigation`'s `useRouter` on the Next.js apps) —
   * `StaffAuthGuard` can't import an app-local routing module, so it's injected instead. */
 ...
 * `StaffAuthRuntimeProvider.tsx`, `useStaffAuth.ts`).
 */
export function createStaffAuthModule({ accessTokenCookie, useRouter }: CreateStaffAuthModuleOptions) {
  ...
  /**
   * Client-side guard for the protected route tree — UX layer only, mirrors storefront's
   * `AuthGuard`. Real authorization must still be enforced by the backend on every protected
