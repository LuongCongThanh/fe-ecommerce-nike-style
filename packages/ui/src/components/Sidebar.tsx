...
function SidebarProvider({
  ...
  // Internal state of the sidebar — `openProp`/`setOpenProp` let a caller control it from outside.
  ...
  // Lets consumers style with `data-state="expanded"|"collapsed"`.
...
function SidebarRail({ className, ...props }: React.ComponentProps<'button'>) {
  ...
  return (
    <button
      ...
      className={cn(
        'hover:after:bg-sidebar-border group-data-[side=left]:-inset-e-4 group-data-[side=right]:inset-s-0 after:inset-s-1/2 absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear after:absolute after:inset-y-0 after:w-0.5 sm:flex',
...
function SidebarGroupAction({ className, asChild = false, ...props }: React.ComponentProps<'button'> & { readonly asChild?: boolean }) {
  ...
  return (
    <Comp
      ...
      className={cn(
        'text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground inset-e-3 absolute top-3.5 flex aspect-square w-5 items-center justify-center rounded-md p-0 outline-hidden transition-transform focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
...
const sidebarMenuButtonVariants = cva(
  'peer/menu-button ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground data-[active=true]:bg-sidebar-accent data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-hidden transition-[width,height,padding] group-has-data-[sidebar=menu-action]/menu-item:pe-8 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:font-medium group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! focus-visible:ring-2 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0',
  {
    variants: {
      variant: {
...
function SidebarMenuAction({
  ...
  showOnHover = false,
  ...props
}: React.ComponentProps<'button'> & { readonly asChild?: boolean; readonly showOnHover?: boolean }) {
  const Comp = asChild ? Slot : 'button';
  ...
  return (
    <Comp
      ...
      className={cn(
        'text-sidebar-foreground ring-sidebar-ring peer-hover/menu-button:text-sidebar-accent-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground inset-e-1 absolute top-1.5 flex aspect-square w-5 items-center justify-center rounded-md p-0 outline-hidden transition-transform focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
        'after:absolute after:-inset-2 md:after:hidden',
...
function SidebarMenuBadge({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'text-sidebar-foreground inset-e-1 pointer-events-none absolute flex h-5 min-w-5 items-center justify-center rounded-md px-1 text-xs font-medium tabular-nums select-none',
...
function SidebarMenuSkeleton({ className, showIcon = false, ...props }: React.ComponentProps<'div'> & { readonly showIcon?: boolean }) {
...
function SidebarMenuSubButton({
  ...
  className,
  ...props
}: React.ComponentProps<'a'> & { readonly asChild?: boolean; readonly size?: 'sm' | 'md'; readonly isActive?: boolean }) {
