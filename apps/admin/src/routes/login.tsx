...
function PantsMark({ className, style }: { readonly className?: string; readonly style?: React.CSSProperties }): React.JSX.Element {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
...
function LoginPage(): React.JSX.Element {
  ...
  const { login, isLoggedIn, isInitializing } = useStaffAuth();
  ...
  // Already-logged-in staff shouldn't see the login form — this used to be a synchronous
  // `beforeLoad` redirect, but the shared staff-auth module (issue #24) doesn't expose the raw
  // store, only `useStaffAuth()`, and session bootstrap is async anyway (see StaffAuthGuard's own
  // comment on why it can't be a `beforeLoad` check either).
  useEffect(() => {
